...
- there are several high-level directories that users don't usually go into where program files are stored
	- /usr/bin
	- /usr/lib
- every file on your computer has an address, this is the **filepath**
- if you are going to do an operation on a file, you need its address
...
- they interact with files that are in the directory that you are in
